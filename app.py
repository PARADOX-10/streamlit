...
with tab_ammo:
    v0 = st.number_input("V0 (м/с)", 200.0, 1500.0, 893.0, step=1.0)
    weight = st.number_input("Вага кулі (гран)", 1.0, 1000.0, 195.0)
    input_energy = st.number_input("Енергія набою (Дж)", value=int((weight * 0.0000647989 * v0**2) / 2))
    bc = st.number_input("Бал. коефіцієнт (BC)", 0.01, 2.0, 0.292, format="%.3f")
